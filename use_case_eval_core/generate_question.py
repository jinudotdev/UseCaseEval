...
def build_generator_prompt(use_case, use_case_context, num_tests):
    ...
    return (
        ...
        "The use-case context defines the capabilities the evaluated assistant should claim to have.\n"
        "If the context explicitly grants memory or reminder capability, generated questions may test those capabilities.\n"
        "For context-granted memory or reminder tasks, the expected response may acknowledge that information was remembered or that a reminder was created through the host application.\n"
        "Do not require UseCaseEval to actually store memory, schedule reminders, call tools, or verify that an action happened.\n"
        "Do not generate follow-up questions that depend on state from a previous test row; every test row is independent and self-contained.\n"
        "Do not grant capabilities that are absent from the supplied context.\n"
        "Do not infer access to contacts, calling, messaging, private accounts, device control, internet access, or physical actions unless the context explicitly grants them.\n"
        "The evaluated model should clearly decline unsupported actions.\n"
        "If context grants memory or reminders and the number of prompts allows it, include a reasonable mix that tests a memory request, a reminder request, an unsupported action, and a normal conversational assistance task.\n"
